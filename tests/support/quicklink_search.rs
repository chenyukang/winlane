fn verify_quicklink_search(mtm: MainThreadMarker) {
    let delegate = Delegate::new(mtm);
    let state = delegate.ivars();
    state.catalog_checked.set(Some(Instant::now()));
    state.mode.set(Some(PanelMode::Search));
    state.config.borrow_mut().quicklinks = vec![winlane::quicklinks::Quicklink {
        id: "rust-docs".into(),
        name: "Rust docs".into(),
    ...
    crate::quicklink_ui::verify_editor(mtm);
...
fn verify_inline_quicklink(mtm: MainThreadMarker) {
    ...
    for density in [DisplayDensity::Compact, DisplayDensity::Normal] {
        ...
        let link = winlane::quicklinks::Quicklink {
            id: "many".into(),
        ...
        delegate.begin_quicklink_input(
            link.clone(),
            winlane::quicklinks::Template::parse(&link.link).unwrap(),
        );
        let bar = ui.quicklink_bar.borrow();
        for i in 0..8 {
    ...
    let link = winlane::quicklinks::Quicklink { id: "choice".into(), name: "Choice".into(), link: r#"https://example.com/{argument name="Tone" type="choice" options="Friendly\nFormal" default="Formal"}"#.into(), open_with: String::new() };
    delegate.begin_quicklink_input(
        link.clone(),
        winlane::quicklinks::Template::parse(&link.link).unwrap(),
    );
