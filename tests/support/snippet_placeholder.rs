pub fn verify_dates(builder: &PlaceholderBuilder) {
    ...
    assert_eq!(builder.preset.numberOfItems() as usize, DATE_PRESETS.len() + 1);
    ...
    let timestamp = render(&Template::parse("{timestamp}").unwrap(), "", &HashMap::new()).unwrap().parse::<i64>().unwrap();
    let now = objc2_foundation::NSDate::now().timeIntervalSince1970() as i64;
    assert!((now - timestamp).abs() <= 2);
    builder.preset.selectItemAtIndex(DATE_PRESETS.len() as isize);
    builder.format.setStringValue(ns_string!("yyyy-MM-dd 'at' HH:mm"));
    builder.update();
...
pub fn verify_fields(builder: &PlaceholderBuilder) -> String {
    ...
    builder.options.setString(ns_string!("Friendly\nFormal\nChoose…"));
