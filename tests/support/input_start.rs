fn verify_input_language_is_prepared_before_focus(mtm: MainThreadMarker) {
...
fn verify_search_composition_survives_refresh(mtm: MainThreadMarker) {
    ...
    let delegate = responsive_fixture(mtm);
    delegate.ivars().config.borrow_mut().input_method = winlane::input_method::InputMethod::Chinese;
...
fn verify_direct_layout_input(mtm: MainThreadMarker) {
    use objc2_foundation::{NSNotFound, NSRange};
    let before = Source::current(mtm).and_then(|source| source.id());
    let delegate = responsive_fixture(mtm);
