fn verify_projects_search(mtm: MainThreadMarker) {
    use winlane::projects::{Cache, Kind, Project};
    let temporary =
    ...
    let available = crate::project_open::application_path().is_some();
    for (path, kind) in [(folder, Kind::Folder), (workspace, Kind::Workspace)] {
        ...
        let prepared = crate::project_open::PreparedProject::new(&project);
        if available {
