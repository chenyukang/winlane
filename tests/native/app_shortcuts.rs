use super::*;
use crate::macos::platform::applications::{launch, resolve_application};
...
pub fn verify_hidden_settings(target: &AnyObject, mtm: MainThreadMarker) {
    ...
    let general = crate::macos::ui::settings::SettingsWindow::new(target, mtm);
    general.fill(&config);
...
pub fn verify_autosave_target(
    ...
    saved: impl Fn() -> winlane::core::config::Config,
) {
...
pub fn verify_background_launch() {
    ...
    let wake = crate::macos::platform::main_wake::MainWake::new(mtm, || {});
    ...
    let pid = wait_for(crate::macos::app::tests::launch_search_fixture(
        target.clone(),
    ));
