...
pub fn verify(mtm: MainThreadMarker) {
    ...
    app.config.borrow_mut().auto_appclose = settings.clone();
    app.auto_appclose.borrow_mut().configure(&settings);
    app.auto_appclose.borrow_mut().next_scan = Some(Instant::now() + Duration::from_secs(3600));

    let (tx, rx) = mpsc::channel();
    let token = Arc::new(AtomicBool::new(false));
    let generation = app.auto_appclose.borrow().generation;
    app.auto_appclose.borrow_mut().job =
    ...
    assert_eq!(
        ...
        appclose::CloseResult::Skipped,
        "cancelled work never calls AX"
    );
    delegate.poll_auto_appclose();
    assert!(
        matches!(app.auto_appclose.borrow().job, Some(Job::Close(..))),
        "only one job may run"
    );
    tx.send(appclose::CloseResult::Requested).unwrap();
    delegate.poll_auto_appclose();
    assert_eq!(
        app.auto_appclose.borrow().planner.pending().count(),
        1,
        "a completed request remains pending even if cancellation arrived later"
    );

    let (scan_tx, scan_rx) = mpsc::channel();
    app.auto_appclose.borrow_mut().job = Some(Job::Scan(scan_rx, generation));
    scan_tx
        .send(appclose::Scan {
            apps: vec![],
        ...
        .unwrap();
    delegate.poll_auto_appclose();
    assert_eq!(app.auto_appclose.borrow().planner.pending().count(), 0);
    ...
    off.enabled = false;
    app.config.borrow_mut().auto_appclose = off.clone();
    app.auto_appclose.borrow_mut().configure(&off);
    assert!(
        token.load(Ordering::Acquire),
        "global off cancels pending actions immediately"
    );
    assert_eq!(app.auto_appclose.borrow().settings.rules, settings.rules);
    app.auto_appclose.borrow_mut().job.take();
    ...
    delegate.poll_auto_appclose();
    assert!(
        app.auto_appclose.borrow().job.is_none(),
        "late scan cannot restart disabled auto-appclose"
    );
    assert!(!app.auto_appclose.borrow().settings.enabled);
    println!(
        "Auto AppClose: MRU cancellation, global off, late workers, one in-flight operation and close confirmation verified; no real windows closed."
    );
}
...
fn verify_interval(mtm: MainThreadMarker, settings: &Settings, target: &Target) {
    ...
    let next = app.auto_appclose.borrow().next_scan.unwrap();
    assert!(next >= before + Duration::from_secs(10) && next <= after + Duration::from_secs(10));
    assert!(matches!(
        app.auto_appclose.borrow().job,
        Some(Job::Scan(..))
    ));
    ...
    app.config.borrow_mut().auto_appclose = slower.clone();
    let before = Instant::now();
    app.auto_appclose.borrow_mut().configure(&slower);
    let after = Instant::now();
    let state = app.auto_appclose.borrow();
