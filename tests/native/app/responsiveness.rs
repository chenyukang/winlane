...
pub(super) fn responsive_fixture(mtm: MainThreadMarker) -> Retained<Delegate> {
...
pub(super) fn pending_test_focus(delegate: &Delegate, pid: i32) -> mpsc::Sender<Option<u64>> {
    let state = delegate.ivars();
...
pub(super) fn verify_responsive_panels(mtm: MainThreadMarker) {
...
pub(super) fn verify_async_focus_order(mtm: MainThreadMarker) {
...
pub(super) fn verify_async_window_snapshot(mtm: MainThreadMarker) {
    let delegate = responsive_fixture(mtm);
    let state = delegate.ivars();
    state.demo.set(false);
