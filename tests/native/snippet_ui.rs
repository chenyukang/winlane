...
pub fn verify_editor(target: &AnyObject, mtm: MainThreadMarker) {
    ...
    let editor = SnippetEditor::new(
        Vec::new(),
        Box::new(move |snippets| {
            saved.replace(snippets);
            Ok(())
        }),
        mtm,
    );
    assert_eq!(
        ...
        windows_before,
        "the snippet editor must not create a window"
    );
    let settings = crate::macos::ui::settings::SettingsWindow::new(target, mtm);
    ...
    editor.ui().body.setString(ns_string!(
        "Hello {argument name=\"Name\"}, today is {date}.\n{clipboard}"
    ));
    editor.changed();
    ...
    editor
        .ui()
        .body
        .setString(ns_string!("{argument name=bad}"));
    editor.changed();
    assert_eq!(
        *stored.borrow(),
        initial,
        "invalid draft must retain the last saved body"
    );
    editor.add(sel!(addSnippet:), None);
    ...
    assert_eq!(
        stored.borrow().len(),
        2,
        "invalid draft must not block saving other snippets"
    );
    ...
    editor
        .ui()
        .body
        .setString(&NSString::from_str(&initial[0].body));
    editor.changed();
    ...
    crate::macos::ui::snippets::placeholder::tests::verify_dates(&editor.ui().builder);
    ...
    editor.insert_placeholder(sel!(insertPlaceholder:), None);
    let token = crate::macos::ui::snippets::placeholder::tests::verify_fields(&editor.ui().builder);
    editor.commit_placeholder(sel!(commitSnippetPlaceholder:), None);
    assert!(editor.ui().builder.view.isHidden());
    assert_eq!(
        stored.borrow()[0].body,
        format!("{token}{body_before_builder}")
    );
    assert_eq!(
        NSApplication::sharedApplication(mtm).windows().len(),
        windows_before_builder
    );
    ...
    let ArgumentControl::Text(field) = &form.ivars().fields.borrow()[0] else {
    ...
    assert!(!&host.isVisible());
    println!(
        "Snippet editor, autosave, invalid drafts, repeated arguments, preview and Escape checks passed."
    );
}
...
fn verify_typed_arguments(mtm: MainThreadMarker) {
    ...
    );
    {
        ...
        let ArgumentControl::Multiline(notes) = &fields[0] else {
            panic!("expected multiline")
        };
        assert_eq!(notes.string().to_string(), "Line 1\nLine 2");
        notes.setString(ns_string!("first\nsecond"));
        let ArgumentControl::Choice { popup, .. } = &fields[1] else {
            panic!("expected dropdown")
        };
        popup.selectItemAtIndex(2);
        let ArgumentControl::Text(extra) = &fields[2] else {
            panic!("expected optional text")
        };
    ...
    let tokens: Vec<_> = (0..8)
        ...
        .collect();
    let large = SnippetArguments::new(
        "Large form",
    ...
    assert_eq!(
        large.window().contentView().unwrap().frame().size.height,
        600.0
    );
    ...
    let scroll = root
        ...
        .iter()
        .find_map(|view| {
            view.downcast::<NSScrollView>()
                .ok()
                .filter(|view| view.frame().origin.y == 236.0)
        })
        .unwrap();
    ...
    if let Ok(directory) = std::env::var("WINLANE_PREVIEW_DIR") {
        ...
        std::fs::create_dir_all(&directory).unwrap();
        assert!(png.writeToFile_atomically(
            &NSString::from_str(&format!("{directory}/snippet-typed-fields.png")),
            true
        ));
    }
}
