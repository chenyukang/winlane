...
每个有候选窗口的应用会按安装包里的英文名称自动获得一个或两个小写字母，显示在候选行左侧。同一应用的多个窗口共用 alias，仍分别显示。优先使用首字母；冲突时尝试词首字母和名称中的两字母组合，然后分配未占用的字母组合。按当前约定，`w` 保留给 WeChat，因此 Warp 通常获得 `wa`。
...
- 切换模式优先匹配当前列表中完整 alias 对应的应用。如果没有对应窗口，则按 alias 前缀查找：例如保存了 `z → Zed`、`zu → Zulip`，当前只有 Zulip 的窗口时，输入 `z` 即可定位 Zulip；有多个前缀匹配应用时，需要继续输入第二个字母。同一应用的多个窗口不算冲突，定位列表中的首个窗口。
- 切换时 Backspace 删除字母，Tab 或方向键清空 alias 并继续选择。输入第三个字母会开始新的 alias。没有匹配窗口或前缀仍有多个候选应用时，松键会取消，不会切换到之前高亮的窗口。已有 alias 分配保持不变。
