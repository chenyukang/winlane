...
Loading happens on a background worker after entering the command, so you can type while it loads. Chrome may keep its history database locked. Winlane reads a temporary private copy of `History` together with its SQLite write-ahead/rollback journal, checks for changes during copying, and lets SQLite recover committed data in that copy. It never writes to Chrome's files. The copy is deleted when the read finishes. Individual source files larger than 256 MB are skipped with an error; other readable profiles still appear.

Results stay in memory only while this list is open and are cleared when you leave. No separate history library is saved, and no webpage or favicon is fetched during search. Chrome may take a moment to flush its latest visit to disk.
