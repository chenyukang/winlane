...
Enable **Settings → General → Debug logging** for diagnostic detail; changes take effect immediately. The default is off. Normal operation events and warnings are still recorded. Logs append to `~/Library/Logs/Winlane/winlane.log`, with one rotated `winlane.previous.log` (about 2 MiB each). **Open Logs Folder** reveals them in Finder. Entries contain local timestamps, severity, component/event names, process/window IDs and app bundle IDs; window titles, search text and clipboard contents are not logged. A bounded background queue keeps disk writes off the UI thread, with a `dropped` count if it fills. Existing window-order and Accessibility diagnostics share this switch and file. This replaces the old `WinlaneTraceRecency` preference and `WINDOWLANE_DEBUG_WINDOWS` environment switch.
