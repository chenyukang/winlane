...
For input-start timing diagnostics, launch with `WINLANE_TRACE_INPUT=1`. The first ten search-shortcut invocations record at most two seconds and 300 state transitions each to `/tmp/winlane-input-trace-<pid>.log`, with owner-only permissions. The trace includes focus, selected input-source IDs, composition state, and event timing, but no key codes or typed text. Diagnostics are disabled by default and are not a saved preference.
