...
Configure **Settings → General → Logging** to choose Debug, Info (default), Warn, Error, or Off. Each level includes higher-severity events; Off disables all entries. Existing Debug logging preferences migrate to Debug or Info. The log file defaults to `~/Library/Logs/Winlane/winlane.log`; enter an absolute path or a path starting with `~/`, or leave the field blank to restore the default. Changes apply immediately. A new path is checked before saving, so an unwritable destination leaves the previous configuration active. Existing log files remain in their original location.

Logs append across restarts, with one rotated file beside the configured log (`winlane.previous.log` for the default filename, about 2 MiB per file). **Open Logs Folder** opens the configured directory in Finder. Entries contain local timestamps, severity, component/event names, process/window IDs and app bundle IDs; window titles, search text and clipboard contents are not logged. A bounded background queue keeps event writes off the UI thread, with a `dropped` count if it fills. Window-order and Accessibility diagnostics use the same level and file. This replaces the old `WinlaneTraceRecency` preference and `WINDOWLANE_DEBUG_WINDOWS` environment switch.
...
Settings creates each page when it is first selected. Pages that have not been opened retain their saved configuration without allocating controls. Closing Settings releases its window, pages, and Snippet/Quicklink editors after the close callback returns. Unfinished snippet and quicklink drafts remain as plain data for the current app session; reopening restores the last selected page and creates other editors only when visited. The native checks cover lazy creation, configuration preservation, autosave on close, and repeated release/reopen using weak references to the window and editors.
