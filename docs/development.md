...
All search scopes and inline Quicklink arguments share the search input-source preference, which defaults to English. Restoring the last source used in Winlane requires explicitly choosing Last Used. Preparing a command's search before presenting its panel preserves the selected startup source; it is cancelled only after a focused search loses focus or the session ends.
