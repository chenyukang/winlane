...
Press **Esc** to close the picker. The **‹ Snippets** button returns to ordinary search. Backspace only deletes text; an empty query stays in Snippets. Spaces stay part of the query in snippet search. Ordinary search and switch mode do not mix in snippets, so window aliases keep their usual behavior. Entering snippet search preserves the application you were using as the paste destination.
